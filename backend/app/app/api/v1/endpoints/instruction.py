...
async def get_instruction_list(
    ...
    """
    Gets a paginated list of instructions
    """
    query = (
        select(ExamInstruction)
        .options(
            selectinload(ExamInstruction.exam_papers),  # Load related exam paper
            selectinload(ExamInstruction.created_by),  # Load creator details
        )
    )
