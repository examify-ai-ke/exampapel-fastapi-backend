...
from app.schemas.exam_paper_schema import ExamDescriptionCreate, ExamDescriptionUpdate
...
class CRUDExamDescription(
    CRUDBase[ExamDescription, ExamDescriptionCreate, ExamDescriptionUpdate]
):

    async def get_exam_description_by_slug(
        self, *, slug: str, db_session: AsyncSession | None = None
    ) -> ExamDescription:
        db_session = db_session or super().get_db().session
        desc = await db_session.execute(
            select(ExamDescription).where(col(ExamDescription.slug).ilike(f"%{slug}%"))
        )
        return desc.unique().scalars().all()
...
exam_description = CRUDExamDescription(ExamDescription)
