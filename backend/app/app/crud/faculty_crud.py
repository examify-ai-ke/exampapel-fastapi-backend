...
class CRUDFaculty(CRUDBase[Faculty, FacultyCreate, FacultyUpdate]):
    ...
    async def create(
        self, *, obj_in: FacultyCreate, created_by_id: str = None, db_session: AsyncSession | None = None
    ) -> Faculty:
    ...
    async def update(
        self, *, obj_new: FacultyUpdate, obj_current: Faculty, db_session: AsyncSession | None = None
    ) -> Faculty:
        db_session = db_session or super().get_db().session
