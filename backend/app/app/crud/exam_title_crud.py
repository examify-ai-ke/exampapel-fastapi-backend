...
from app.schemas.exam_paper_schema import ExamTitleCreate, ExamTitleUpdate
...
class CRUDExamTitle(CRUDBase[ExamTitle, ExamTitleCreate, ExamTitleUpdate]):

    async def get_exam_title_by_slug(
        self, *, slug: str, db_session: AsyncSession | None = None
    ) -> ExamTitle:
        db_session = db_session or super().get_db().session
        title = await db_session.execute(
            select(ExamTitle).where(col(ExamTitle.slug).ilike(f"%{slug}%"))
        )
        return title.unique().scalars().all()
...
exam_title = CRUDExamTitle(ExamTitle)
