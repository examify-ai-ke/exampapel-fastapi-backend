...
class CRUDModule(CRUDBase[Module, ModuleCreate, ModuleUpdate]):
    ...
    async def create(
        self, *, obj_in: ModuleCreate, created_by_id: str = None, db_session: AsyncSession | None = None
    ) -> Module:
    ...
    async def update(
        self, *, obj_new: ModuleUpdate, obj_current: Module, db_session: AsyncSession | None = None
    ) -> Module:
        db_session = db_session or super().get_db().session
