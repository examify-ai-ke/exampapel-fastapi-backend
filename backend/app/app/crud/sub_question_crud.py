...
from app.schemas.question_schema import SubQuestionCreate, SubQuestionUpdate
...
class CRUDSubQuestion(CRUDBase[SubQuestion, SubQuestionCreate, SubQuestionUpdate]):

    async def get_sub_question_by_slug(
        self, *, slug: str, db_session: AsyncSession | None = None
    ) -> SubQuestion:
        db_session = db_session or super().get_db().session
        q_set = await db_session.execute(
            select(SubQuestion).where(col(SubQuestion.slug).ilike(f"%{slug}%"))
        )
...
sub_question = CRUDSubQuestion(SubQuestion)
